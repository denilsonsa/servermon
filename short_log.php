...
<?php
if( $datestamp ) echo '<p class="testtime">Last change at <span class="time">'.$date_modified_nice.'</span></p>'.NEWLINE;
?>
</div>
<?php
...
<?php
	...
	foreach( array('','.1') as $suffix ) {
		...
		foreach( array_reverse($v) as $s ) {
			echo '<tr class="'. ($even_odd?'evenline':'oddline') .' '. ($s['ok']==0?'success':'fail') . ' important">';
